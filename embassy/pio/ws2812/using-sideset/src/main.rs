    ...
    let prg = pio_asm!(
        ...
        .wrap_target
            bit_loop:
                ...
                jmp !x do_zero  side 1 [1]         ; Branch on the bit. Drive the pin HIGH.
            do_one:
                jmp bit_loop    side 1 [3]         ; If the bit is 1, keep the pin HIGH longer.
            do_zero:
                nop             side 0 [3]         ; If the bit is 0, drive the pin LOW.
        .wrap
        "
    );
